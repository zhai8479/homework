 ...
 * @return bool
 */
function get_login_status ($db, &$user_id) {
    /**
     ...
     * 4. 返回登陆状态
     */
    $user_id = empty($_SERVER['HTTP_USER_ID'])?null:trim($_SERVER['HTTP_USER_ID']);
    $token = empty($_SERVER['HTTP_TOKEN'])?null:trim($_SERVER['HTTP_TOKEN']);
    if (empty($user_id) || empty($token)) {
        $is_login = false;
    } else {
        // 验证token是否正确
        $result = $db->query("select * from `users` where `id` = $user_id and `token` = '$token'");
        if ($result) {
